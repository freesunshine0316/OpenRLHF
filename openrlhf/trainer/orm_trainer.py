...
class OutcomeRewardModelTrainer(ABC):
    """
    Trainer for training a outcome reward model.
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    inputs, attention_masks, labels, action_mask = data
                ...
                orm_loss = self.loss_fn(scores, labels, action_mask)
                loss = orm_loss + aux_loss * self.args.aux_loss_coef
                self.strategy.backward(loss, self.model, self.optimizer)
                self.strategy.optimizer_step(self.optimizer, self.model, self.scheduler)

                loss_sum += loss.item()
                logs_dict = {
                    "orm_loss": orm_loss.item(),
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                loss = self.loss_fn(scores, labels, action_mask)

                times += 1
                loss_sum += loss.item()
                bar_dict = {"eval orm_loss": loss_sum / times}
                step_bar.update()
