...
def remote_rm_fn(api_url, sequences, queries, responses, score_key="rewards"):
    """remote reward model API
    api_url: RM API, We assume that the API supports two modes: merging query + response and not merging
    sequences: query+response with the template
    queries: query only
    responses: response only
    design is made optional.
    score_key: RM score key
    """
    scores = request_api_wrapper(api_url, {"sequence": sequences, "query": queries, "response": responses}, score_key)
    return torch.tensor(scores)


@ray.remote
def remote_rm_fn_ray(api_url, sequences, queries, responses, score_key="rewards"):
    return remote_rm_fn(api_url, sequences, queries, responses, score_key)
