...
from openrlhf.search_algorithm.search_utils import DEFAULT_N
...
def verify_by_lean(api_url, statements: List[str], proofs: List[str]):
    ...
    scores = request_api_wrapper(api_url, {"query": queries})
    return scores

def longest_partial_trajectory(traj: str, error_positions: List[dict], include_error_line=False) -> str:
    ...
    if include_error_line and error_line <= len(lines):
